...
const InquiryCancelReasonPage: NextPage = () => {
  // ? useSession
  ...
  // ? useQuery
  const { data, isLoading, refetch } = api.inquiryCancelReasons.getOne.useQuery(
    {
      id: id?.toString() || ''
    },
    {
  ...
  const { mutateAsync, isLoading: isSaving } =
    api.inquiryCancelReasons.createOrUpdateOne.useMutation()
  const { mutateAsync: deleteAsync, isLoading: isDeleting } =
    api.inquiryCancelReasons.deleteOne.useMutation()
  ...
  return (
    <Layout
      ...
      breadcrumbs={[
        { label: 'Home', link: '/' },
        { label: 'Inquiry Cancel Reasons', link: '/inquiry-cancel-reasons' },
        {
          label: id === 'new' ? 'New' : data?.name || 'Loading'
        }
      ]}
      title={`Inquiry Cancel Reason - ${data?.name || id}`}
    >
      <Card className="my-2">
        <Form
          form={form}
          onFinish={async formData => {
            try {
              ...
              if (id === 'new') {
                router.push(`/inquiry-cancel-reasons/${res.id}`)
                notificationApi.success({
                  message: 'Inquiry Cancel Reason created'
                })
              } else {
                refetch()
                notificationApi.success({
                  message: 'Inquiry Cancel Reason updated'
                })
              }
            } catch (err) {
              notificationApi.error({
                message: 'Error saving Inquiry Cancel Reason'
              })
            }
          }}
          layout="vertical"
...
export default InquiryCancelReasonPage
