...
const CurrencyPage: NextPage = () => {
  ...
  // ? useQuery
  const { data, isLoading, refetch } = api.currency.getOne.useQuery(
    {
      id: id?.toString() || ''
    },
    {
  ...
  const { mutateAsync, isLoading: isSaving } =
    api.currency.createOrUpdateOne.useMutation()
  const { mutateAsync: deleteAsync, isLoading: isDeleting } =
    api.currency.deleteOne.useMutation()
  ...
  return (
    <Layout
      ...
      breadcrumbs={[
        { label: 'Home', link: '/' },
        { label: 'Currencies', link: '/currency' },
        {
          label: id === 'new' ? 'New' : data?.name || 'Loading'
        }
      ]}
      title={`Currency - ${data?.name || id}`}
    >
      <Card className="my-2">
        <Form
          form={form}
