...
const NotifyPartiesPage: NextPage = () => {
  // ? useSession
  ...
  // ? useQuery
  const { data, isLoading } = api.notifyParty.getAll.useQuery(variables, {
    enabled: !!session
  })

  return (
    <Layout
      breadcrumbs={[{ label: 'Home', link: '/' }, { label: 'Notify Party' }]}
      title="Notify Party"
    >
      ...
      <Table
        ...
        columns={[
          ...
          },
          {
            ...
            dataIndex: 'name',
            render: (name, record) => (
              <Link href={`/notify-parties/${record.id}`}>
...
export default NotifyPartiesPage
