...
const NotifyPartyPage: NextPage = () => {
  // ? useSession
  ...
  // ? useQuery
  const { data, isLoading, refetch } = api.notifyParty.getOne.useQuery(
    {
      id: id?.toString() || ''
    },
    {
  ...
  const { mutateAsync, isLoading: isSaving } =
    api.notifyParty.createOrUpdateOne.useMutation()
  const { mutateAsync: deleteAsync, isLoading: isDeleting } =
    api.notifyParty.deleteOne.useMutation()
  ...
  return (
    <Layout
      ...
      breadcrumbs={[
        { label: 'Home', link: '/' },
        { label: 'Notify Parties', link: '/notify-parties' },
        {
          label: id === 'new' ? 'New' : data?.name || 'Loading'
        }
      ]}
      title={`Notify Party - ${data?.name || id}`}
    >
      <Card className="my-2">
        <Form
          form={form}
          onFinish={async formData => {
            try {
              ...
              if (id === 'new') {
                router.push(`/notify-parties/${res.id}`)
                notificationApi.success({
                  message: 'Notify Party created'
                })
              } else {
                refetch()
                notificationApi.success({
                  message: 'Notify Party updated'
                })
              }
            } catch (err) {
              notificationApi.error({
                message: 'Error saving Notify Party'
              })
            }
          }}
          layout="vertical"
...
export default NotifyPartyPage
