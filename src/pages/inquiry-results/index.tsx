...
const InquiryResultsPage: NextPage = () => {
  // ? useSession
  ...
  // ? useQuery
  const { data, isLoading, refetch } = api.inquiryResults.getAll.useQuery(
    variables,
    {
      enabled: !!session
    }
  ...
  const {
    mutateAsync: exportInquiriesResult,
    isLoading: exportInquiriesResultLoading
  } = api.inquiryResults.export.useMutation()
  const { mutateAsync: deleteAttachment } =
    api.attachments.deleteOne.useMutation()
  const {
    mutateAsync: importInquiryResults,
    isLoading: importInquiryResultsLoading
  } = api.inquiryResults.import.useMutation()

  return (
    <Layout
      breadcrumbs={[{ label: 'Home', link: '/' }, { label: 'Inquiry Results' }]}
    ...
    >
      ...
      <Table
        ...
        columns={[
          ...
          },
          {
            ...
            dataIndex: 'name',
            render: (name, record) => (
              <Link href={`/inquiry-results/${record.id}`}>
        ...
        dataSource={data?.inquiryResults}
        rowKey="id"
        pagination={{
        ...
        caption={
          <Space>
            <Button
              type="primary"
              loading={exportInquiriesResultLoading}
              icon={<DownloadOutlined />}
              onClick={async () => {
                const result = await exportInquiriesResult({
            ...
            <Button
              ...
              type="primary"
              loading={importInquiryResultsLoading}
              icon={<DownloadOutlined />}
      ...
      <Modal
        ...
        onOk={async () => {
          ...
          const result = await importInquiryResults({
        ...
        confirmLoading={importInquiryResultsLoading}
      >
        <div className="py-5">
          <Upload
            multiple={false}
...
export default InquiryResultsPage
