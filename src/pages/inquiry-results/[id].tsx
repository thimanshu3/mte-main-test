...
const InquiryResultPage: NextPage = () => {
  // ? useSession
  ...
  // ? useQuery
  const { data, isLoading, refetch } = api.inquiryResults.getOne.useQuery(
    {
      id: id?.toString() || ''
    },
    {
  ...
  const { mutateAsync, isLoading: isSaving } =
    api.inquiryResults.createOrUpdateOne.useMutation()
  const { mutateAsync: deleteAsync, isLoading: isDeleting } =
    api.inquiryResults.deleteOne.useMutation()
  ...
  return (
    <Layout
      ...
      breadcrumbs={[
        { label: 'Home', link: '/' },
        { label: 'Inquiry Results', link: '/inquiry-results' },
        {
          label: id === 'new' ? 'New' : data?.name || 'Loading'
        }
      ]}
      title={`Inquiry Result - ${data?.name || id}`}
    >
      <Card className="my-2">
        <Form
          form={form}
          onFinish={async formData => {
            try {
              ...
              if (id === 'new') {
                router.push(`/inquiry-results/${res.id}`)
                notificationApi.success({
                  message: 'Inquiry Result created'
                })
              } else {
                refetch()
                notificationApi.success({
                  message: 'Inquiry Result updated'
                })
              }
            } catch (err) {
              notificationApi.error({
                message: 'Error saving Inquiry Result'
              })
            }
          }}
          layout="vertical"
