...
const PaymentTermsPage: NextPage = () => {
  // ? useSession
  ...
  // ? useQuery
  const { data, isLoading, refetch } = api.paymentTerms.getAll.useQuery(
    variables,
    {
      enabled: !!session
    }
  ...
  // ? useMutation
  const {
    mutateAsync: exportPaymentTerms,
    isLoading: exportPaymentTermsLoading
  } = api.paymentTerms.export.useMutation()
  const { mutateAsync: deleteAttachment } =
    api.attachments.deleteOne.useMutation()
  const {
    mutateAsync: importPaymentTerms,
    isLoading: importPaymentTermsLoading
  } = api.paymentTerms.import.useMutation()

  return (
    <Layout
      breadcrumbs={[{ label: 'Home', link: '/' }, { label: 'Payment Terms' }]}
      title="Payment Terms"
    >
      ...
      <Table
        ...
        columns={[
          ...
          },
          {
            ...
            dataIndex: 'name',
            render: (name, record) => (
              <Link href={`/payment-terms/${record.id}`}>
        ...
        caption={
          <Space>
            <Button
              type="primary"
              loading={exportPaymentTermsLoading}
              icon={<DownloadOutlined />}
              onClick={async () => {
                const result = await exportPaymentTerms({
            ...
            <Button
              ...
              type="primary"
              loading={importPaymentTermsLoading}
              icon={<DownloadOutlined />}
      ...
      <Modal
        ...
        onOk={async () => {
          ...
          const result = await importPaymentTerms({
        ...
        confirmLoading={importPaymentTermsLoading}
      >
        <div className="py-5">
          <Upload
            multiple={false}
