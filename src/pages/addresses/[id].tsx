...
const AddressPage: NextPage = () => {
  ...
  // ? useQuery
  const { data, isLoading, refetch } = api.address.getOne.useQuery(
    {
      id: id?.toString() || ''
    },
    {
  ...
  const { mutateAsync, isLoading: isSaving } =
    api.address.createOrUpdateOne.useMutation()
  const { mutateAsync: deleteAsync, isLoading: isDeleting } =
    api.address.deleteOne.useMutation()
      ...
      <Card className="my-2">
          ...
          onFinish={async formData => {
            try {
              const res = await mutateAsync({
                ...
                personMobile: formData.personMobile
              })
              if (id === 'new') {
                router.push(`/addresses/${res.id}`)
        ...
        >
          ...
          <Row gutter={16}>
            ...
            <Col span={12}>
              <Form.Item name="personMobile" label="Person Mobile">
                <Input />
              </Form.Item>
            </Col>
          </Row>
