...
const CustomersPage: NextPage = () => {
  ...
  // ? useQuery
  const { data, isLoading, refetch } = api.customers.getAll.useQuery(
    variables,
    {
      enabled: !!session
    }
  ...
  // ? useMutation
  const { mutateAsync: exportCustomers, isLoading: exportCustomersLoading } =
    api.customers.export.useMutation()
  const { mutateAsync: deleteAttachment } =
    api.attachments.deleteOne.useMutation()
  const { mutateAsync: importCustomers, isLoading: importCustomersLoading } =
    api.customers.import.useMutation()

  return (
    <Layout
      breadcrumbs={[{ label: 'Home', link: '/' }, { label: 'Customers' }]}
      title="Customers"
    >
      ...
      <Table
        ...
        caption={
          <Space>
            <Button
              type="primary"
              loading={exportCustomersLoading}
            ...
            <Button
              ...
              type="primary"
              loading={importCustomersLoading}
              icon={<DownloadOutlined />}
              onClick={() => {
                setShowImportModal(1)
              }}
            >
              Import
            </Button>
      ...
      <Modal
        ...
        confirmLoading={importCustomersLoading}
      >
        <div className="py-5">
          <Upload
            multiple={false}
