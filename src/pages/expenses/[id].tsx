...
const ExpensePage: NextPage = () => {
  ...
  // ? useQuery
  const { data, isLoading, refetch } = api.expenses.getOne.useQuery(
    {
      id: id?.toString() || ''
    },
    {
  ...
  const { mutateAsync, isLoading: isSaving } =
    api.expenses.createOrUpdateOne.useMutation()
  const { mutateAsync: deleteAsync, isLoading: isDeleting } =
    api.expenses.deleteOne.useMutation()
