...
const Lut: NextPage = () => {
  ...
  // ? useQuery
  const { data, isLoading, refetch } = api.Lut.getOne.useQuery(
    {
      id: id?.toString() || ''
    },
    {
  ...
  const { mutateAsync, isLoading: isSaving } =
    api.Lut.createOrUpdateOne.useMutation()
  const { mutateAsync: deleteAsync, isLoading: isDeleting } =
    api.Lut.deleteOne.useMutation()
            ...
            {id !== 'new' && session?.user.role !== 'ADMINVIEWER' ? (
              <Button
                ...
                onClick={async () => {
                  ...
                  notificationApi.success({
                    message: `Lut ${data?.deletedAt ? 'restored' : 'deleted'}`
                  })
                  refetch()
                }}
                loading={isDeleting}
