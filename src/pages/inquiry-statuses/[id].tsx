...
const InquiryStatusPage: NextPage = () => {
  // ? useSession
  ...
  // ? useQuery
  const { data, isLoading, refetch } = api.inquiryStatuses.getOne.useQuery(
    {
      id: id?.toString() || ''
    },
    {
  ...
  const { mutateAsync, isLoading: isSaving } =
    api.inquiryStatuses.createOrUpdateOne.useMutation()
  const { mutateAsync: deleteAsync, isLoading: isDeleting } =
    api.inquiryStatuses.deleteOne.useMutation()
  ...
  return (
    <Layout
      ...
      breadcrumbs={[
        { label: 'Home', link: '/' },
        { label: 'Inquiry Statuses', link: '/inquiry-statuses' },
        {
          label: id === 'new' ? 'New' : data?.name || 'Loading'
        }
      ]}
      title={`Inquiry Status - ${data?.name || id}`}
    >
      <Card className="my-2">
        <Form
          form={form}
          onFinish={async formData => {
            try {
              ...
              if (id === 'new') {
                router.push(`/inquiry-statuses/${res.id}`)
                notificationApi.success({
                  message: 'Inquiry Status created'
                })
              } else {
                refetch()
                notificationApi.success({
              ...
              notificationApi.error({
                message: 'Error saving Inquiry Status'
              })
            }
          }}
          layout="vertical"
...
export default InquiryStatusPage
