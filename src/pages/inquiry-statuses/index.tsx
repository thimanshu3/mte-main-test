...
const InquiryStatusesPage: NextPage = () => {
  // ? useSession
  ...
  // ? useQuery
  const { data, isLoading, refetch } = api.inquiryStatuses.getAll.useQuery(
    variables,
    {
      enabled: !!session
    }
  ...
  // ? useMutation
  const {
    mutateAsync: exportInquiriesStatus,
    isLoading: exportInquiriesStatusLoading
  } = api.inquiryStatuses.export.useMutation()
  const { mutateAsync: deleteAttachment } =
    api.attachments.deleteOne.useMutation()
  const {
    mutateAsync: importInquiryStatus,
    isLoading: importInquiryStatusLoading
  } = api.inquiryStatuses.import.useMutation()
  ...
  return (
    ...
    >
      <div className="mb-2 mt-4 flex items-center justify-between">
        ...
        />
        <Link href="/inquiry-statuses/new">
      ...
      <Table
        ...
        columns={[
          ...
          },
          {
            ...
            dataIndex: 'name',
            render: (name, record) => (
              <Link href={`/inquiry-statuses/${record.id}`}>
        ...
        dataSource={data?.inquiryStatuses}
        rowKey="id"
        pagination={{
        ...
        caption={
          <Space>
            <Button
              type="primary"
              loading={exportInquiriesStatusLoading}
              icon={<DownloadOutlined />}
              onClick={async () => {
                const result = await exportInquiriesStatus({
            ...
            <Button
              ...
              type="primary"
              loading={importInquiryStatusLoading}
              icon={<DownloadOutlined />}
      ...
      <Modal
        ...
        onOk={async () => {
          ...
          const result = await importInquiryStatus({
        ...
        confirmLoading={importInquiryStatusLoading}
      >
        <div className="py-5">
          <Upload
            multiple={false}
...
export default InquiryStatusesPage
