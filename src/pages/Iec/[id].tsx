...
const IecCode: NextPage = () => {
  ...
  // ? useQuery
  const { data, isLoading, refetch } = api.IecCode.getOne.useQuery(
    {
      id: id?.toString() || ''
    },
    {
  ...
  const { mutateAsync, isLoading: isSaving } =
    api.IecCode.createOrUpdateOne.useMutation()
  const { mutateAsync: deleteAsync, isLoading: isDeleting } =
    api.IecCode.deleteOne.useMutation()
  ...
  return (
    <Layout
      ...
      breadcrumbs={[
        { label: 'Home', link: '/' },
        { label: 'IecCode', link: '/Iec' },
        {
          label: id === 'new' ? 'New' : data?.name || 'Loading'
        }
      ]}
      title={`IecCode - ${data?.name || id}`}
    >
      <Card className="my-2">
        <Form
          form={form}
          onFinish={async formData => {
            try {
              ...
              if (id === 'new') {
                router.push(`/Iec/${res.id}`)
                notificationApi.success({
                  message: 'IecCode created'
                })
              } else {
                refetch()
                notificationApi.success({
                  message: 'IecCode updated'
                })
              }
            } catch (err) {
              notificationApi.error({
                message: 'Error saving IecCode'
              })
            }
          }}
          layout="vertical"
