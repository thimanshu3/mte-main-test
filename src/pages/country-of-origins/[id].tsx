...
const CountryOfOrigin: NextPage = () => {
  // ? useSession
  ...
  // ? useQuery
  const { data, isLoading, refetch } = api.countryOfOrigin.getOne.useQuery(
    {
      id: id?.toString() || ''
    },
    {
  ...
  const { mutateAsync, isLoading: isSaving } =
    api.countryOfOrigin.createOrUpdateOne.useMutation()
  const { mutateAsync: deleteAsync, isLoading: isDeleting } =
    api.countryOfOrigin.deleteOne.useMutation()
  ...
  return (
    <Layout
      ...
      breadcrumbs={[
        { label: 'Home', link: '/' },
        { label: 'Country Of Origins', link: '/country-of-origins' },
        {
          label: id === 'new' ? 'New' : data?.name || 'Loading'
        }
      ]}
      title={`Country Of Origin - ${data?.name || id}`}
    >
      <Card className="my-2">
        <Form
          form={form}
          onFinish={async formData => {
            try {
              ...
              if (id === 'new') {
                router.push(`/country-of-origins/${res.id}`)
                notificationApi.success({
                  message: 'Country Of Origin created'
                })
              } else {
                refetch()
                notificationApi.success({
                  message: 'Country Of Origin updated'
                })
              }
            } catch (err) {
              notificationApi.error({
                message: 'Error saving Country Of Origin'
              })
            }
          }}
          layout="vertical"
        ...
        >
          <ActionRibbon>
            ...
            {id !== 'new' && session?.user.role !== 'ADMINVIEWER' ? (
              <Link href="/country-of-origins/new">
