...
const ExporterDetailsPage: NextPage = () => {
  // ? useSession
  ...
  // ? useQuery
  const { data, isLoading, refetch } = api.exporterDetails.getOne.useQuery(
    {
      id: id?.toString() || ''
    },
    {
  ...
  const { mutateAsync, isLoading: isSaving } =
    api.exporterDetails.createOrUpdateOne.useMutation()
  const { mutateAsync: deleteAsync, isLoading: isDeleting } =
    api.exporterDetails.deleteOne.useMutation()
  ...
  return (
    <Layout
      ...
      breadcrumbs={[
        { label: 'Home', link: '/' },
        { label: 'Exporter Details', link: '/exporter-details' },
        {
          label: id === 'new' ? 'New' : data?.name || 'Loading'
        }
      ]}
      title={`Exporter Details - ${data?.name || id}`}
    >
      <Card className="my-2">
        <Form
          form={form}
          onFinish={async formData => {
            try {
              ...
              if (id === 'new') {
                router.push(`/exporter-details/${res.id}`)
                notificationApi.success({
                  message: 'Exporter Details created'
                })
              } else {
                refetch()
                notificationApi.success({
                  message: 'Exporter Details updated'
                })
              }
            } catch (err) {
              notificationApi.error({
                message: 'Error saving Exporter Details'
              })
            }
          }}
          layout="vertical"
        ...
        >
          <ActionRibbon>
            ...
            {id !== 'new' && session?.user.role !== 'ADMINVIEWER' ? (
              <Link href="/exporter-details/new">
