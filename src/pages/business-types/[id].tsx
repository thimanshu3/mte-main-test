...
const BusinessTypePage: NextPage = () => {
  ...
  // ? useQuery
  const { data, isLoading, refetch } = api.businessTypes.getOne.useQuery(
    {
      id: id?.toString() || ''
    },
    {
  ...
  const { mutateAsync, isLoading: isSaving } =
    api.businessTypes.createOrUpdateOne.useMutation()
  const { mutateAsync: deleteAsync, isLoading: isDeleting } =
    api.businessTypes.deleteOne.useMutation()
  ...
  return (
    <Layout
      ...
      breadcrumbs={[
        { label: 'Home', link: '/' },
        { label: 'Business Types', link: '/business-types' },
        {
          label: id === 'new' ? 'New' : data?.name || 'Loading'
        }
      ]}
      title={`Business Type - ${data?.name || id}`}
    >
      <Card className="my-2">
        <Form
          form={form}
          onFinish={async formData => {
            try {
              ...
              if (id === 'new') {
                router.push(`/business-types/${res.id}`)
              ...
              notificationApi.error({
                message: 'Error saving Business Type'
              })
            }
          }}
          layout="vertical"
