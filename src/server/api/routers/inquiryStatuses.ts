...
export const inquiryStatusesRouter = createTRPCRouter({
  getAll: protectedProcedure
    ...
    .query(async ({ ctx: { prisma }, input }) => {
      const where = input.search
        ...
        : undefined
      const [inquiryStatuses, total] = await Promise.all([
        ...
        prisma.inquiryStatus.count({
          where
        })
      ])
      return { inquiryStatuses, total }
    }),
  ...
  getOne: protectedProcedure
    ...
    .query(async ({ ctx: { prisma }, input }) => {
      return await prisma.inquiryStatus.findUniqueOrThrow({
        where: {
          id: input.id
        },
        include: {
  ...
  createOrUpdateOne: adminProtectedProcedure
    ...
    .mutation(async ({ ctx: { prisma, session }, input }) => {
      if (input.id)
        return await prisma.inquiryStatus.update({
          where: {
            id: input.id
          },
          data: {
      ...
      return await prisma.inquiryStatus.create({
        data: {
          name: input.name,
  ...
  export: protectedProcedure
    ...
    .mutation(async ({ ctx: { prisma }, input }) => {
      ...
      let skip = 0
      const take = 1000
      const allInquiriesStatus: any[] = []
      while (skip < 10000) {
        const inquiryStatus = await prisma.inquiryStatus.findMany({
          where,
          skip,
          take
        })
        if (!inquiryStatus.length) break
        for (let i = 0; i < inquiryStatus.length; i++) {
          const inquiriesStatus = inquiryStatus[i]
          if (!inquiriesStatus) continue
          allInquiriesStatus.push({
            srno: i + 1 + skip,
            ...inquiriesStatus
          })
        }
      ...
      const sheet = book.addWorksheet('Inquiry Statuses')

      sheet.columns = [
        { header: 'INQUIRY STATUS ID', key: 'id', width: 10 },
        { header: 'NAME', key: 'name', width: 32 }
      ]
      sheet.addRows(allInquiriesStatus)
      ...
      const filename = `Inquiry Statuses-${Date.now()}.xlsx`
  ...
  import: adminProtectedProcedure
    ...
    .mutation(async ({ ctx: { prisma, session }, input }) => {
      ...
      const handleError = async () => {
        ...
        const filename = `Error-Import-Inquiry Status-${Date.now()}.xlsx`
        const buffer = await book.xlsx.writeBuffer()
        const url = await storageClient.addFile({
          filename,
          data: Buffer.from(buffer)
