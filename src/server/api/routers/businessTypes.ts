...
export const businessTypesRouter = createTRPCRouter({
  ...
  getOne: protectedProcedure
    ...
    .query(async ({ ctx: { prisma }, input }) => {
      return await prisma.businessType.findUniqueOrThrow({
        where: {
          id: input.id
        },
        include: {
  ...
  export: protectedProcedure
    ...
    .mutation(async ({ ctx: { prisma }, input }) => {
      ...
      const sheet = book.addWorksheet('Business Types')

      sheet.columns = [
        { header: 'BUSINESS TYPE ID', key: 'id', width: 10 },
        { header: 'NAME', key: 'name', width: 32 }
      ]
      sheet.addRows(allBusinessTypes)
      ...
      const filename = `Business Types-${Date.now()}.xlsx`
  ...
  import: adminProtectedProcedure
    ...
    .mutation(async ({ ctx: { prisma, session }, input }) => {
      ...
      const handleError = async () => {
        ...
        const filename = `Error-Import-Business Type-${Date.now()}.xlsx`
        const buffer = await book.xlsx.writeBuffer()
        const url = await storageClient.addFile({
          filename,
          data: Buffer.from(buffer)
