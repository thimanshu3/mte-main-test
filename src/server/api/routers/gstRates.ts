...
export const gstRatesRouter = createTRPCRouter({
  getAll: protectedProcedure
    ...
    .query(async ({ ctx: { prisma }, input }) => {
      const nSearch = Number(input.search)
      const isNumber = !isNaN(nSearch)
      const where = input.search
        ? {
            id: isNumber ? undefined : input.search,
            rate: isNumber ? nSearch : undefined
          }
        : undefined
  ...
  getOne: protectedProcedure
    ...
    .query(async ({ ctx: { prisma }, input }) => {
      return await prisma.gstRate.findUniqueOrThrow({
        where: {
          id: input.id
        },
        include: {
  ...
  export: protectedProcedure
    ...
    .mutation(async ({ ctx: { prisma }, input }) => {
      ...
      while (skip < 10000) {
        ...
        if (!gstRates.length) break
        for (let i = 0; i < gstRates.length; i++) {
          const gstRate = gstRates[i]
          if (!gstRate) continue
          allGstRates.push({
            srno: i + 1 + skip,
            ...gstRate
      ...
      const sheet = book.addWorksheet('GST Rates')

      sheet.columns = [
        { header: 'GST RATE ID', key: 'id', width: 10 },
        { header: 'RATE', key: 'rate', width: 10 }
      ]
      sheet.addRows(allGstRates)
      ...
      const filename = `GST Rates-${Date.now()}.xlsx`
  ...
  import: adminProtectedProcedure
    ...
    .mutation(async ({ ctx: { prisma, session }, input }) => {
      ...
      const handleError = async () => {
        ...
        const filename = `Error-Import-GST Rate-${Date.now()}.xlsx`
        const buffer = await book.xlsx.writeBuffer()
        const url = await storageClient.addFile({
          filename,
          data: Buffer.from(buffer)
