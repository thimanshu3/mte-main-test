...
export const inquiryResultsRouter = createTRPCRouter({
  getAll: protectedProcedure
    ...
    .query(async ({ ctx: { prisma }, input }) => {
      const where = input.search
        ...
        : undefined
      const [inquiryResults, total] = await Promise.all([
        ...
        prisma.inquiryResult.count({
          where
        })
      ])
      return { inquiryResults, total }
    }),
  ...
  getOne: protectedProcedure
    ...
    .query(async ({ ctx: { prisma }, input }) => {
      return await prisma.inquiryResult.findUniqueOrThrow({
        where: {
          id: input.id
        },
        include: {
  ...
  createOrUpdateOne: adminProtectedProcedure
    ...
    .mutation(async ({ ctx: { prisma, session }, input }) => {
      if (input.id)
        return await prisma.inquiryResult.update({
          where: {
            id: input.id
          },
          data: {
      ...
      return await prisma.inquiryResult.create({
        data: {
          name: input.name,
  ...
  export: protectedProcedure
    ...
    .mutation(async ({ ctx: { prisma }, input }) => {
      ...
      let skip = 0
      const take = 1000
      const allInquiriesResults: any[] = []
      while (skip < 10000) {
        const inquiryResults = await prisma.inquiryResult.findMany({
          where,
          skip,
          take
        })
        if (!inquiryResults.length) break
        for (let i = 0; i < inquiryResults.length; i++) {
          const inquiriesResult = inquiryResults[i]
          if (!inquiriesResult) continue
          allInquiriesResults.push({
            srno: i + 1 + skip,
            ...inquiriesResult
          })
        }
      ...
      const sheet = book.addWorksheet('Inquiry Results')

      sheet.columns = [
        { header: 'INQUIRY RESULT ID', key: 'id', width: 10 },
        { header: 'NAME', key: 'name', width: 32 }
      ]
      sheet.addRows(allInquiriesResults)
      ...
      const filename = `Inquiry Results-${Date.now()}.xlsx`
  ...
  import: adminProtectedProcedure
    ...
    .mutation(async ({ ctx: { prisma, session }, input }) => {
      ...
      const handleError = async () => {
        ...
        const filename = `Error-Import-Inquiry Result-${Date.now()}.xlsx`
        const buffer = await book.xlsx.writeBuffer()
        const url = await storageClient.addFile({
          filename,
          data: Buffer.from(buffer)
