...
export const paymentTermsRouter = createTRPCRouter({
  ...
  getOne: protectedProcedure
    ...
    .query(async ({ ctx: { prisma }, input }) => {
      return await prisma.paymentTerm.findUniqueOrThrow({
        where: {
          id: input.id
        },
        include: {
  ...
  createOne: adminProtectedProcedure
    ...
    .mutation(async ({ ctx: { prisma, session }, input }) => {
      return await prisma.paymentTerm.create({
        data: {
          name: input.name,
          conditions: input.conditions
            ? {
                createMany: {
                  data: input.conditions.map((c, i) => ({ ...c, order: i + 1 }))
                }
              }
            : undefined,
  ...
  deleteOne: adminProtectedProcedure
    ...
    .mutation(async ({ ctx: { prisma, session }, input }) => {
      return await prisma.paymentTerm.update({
        where: {
          id: input.id
        },
        data: input.activate
  ...
  export: protectedProcedure
    ...
    .mutation(async ({ ctx: { prisma }, input }) => {
      ...
      while (skip < 10000) {
        ...
        if (!paymentTerms.length) break
        for (let i = 0; i < paymentTerms.length; i++) {
          const paymentTerm = paymentTerms[i]
          if (!paymentTerm) continue
          allPaymentTerms.push({
            srno: i + 1 + skip,
            ...paymentTerm
          })
        }
      ...
      const sheet = book.addWorksheet('Payment Terms')

      sheet.columns = [
        { header: 'PAYMENT TERM ID', key: 'id', width: 10 },
        { header: 'NAME', key: 'name', width: 32 }
      ]
      sheet.addRows(allPaymentTerms)
      ...
      const filename = `Payment Terms-${Date.now()}.xlsx`
  ...
  import: adminProtectedProcedure
    ...
    .mutation(async ({ ctx: { prisma, session }, input }) => {
      ...
      const handleError = async () => {
        ...
        const filename = `Error-Import-Payment Term-${Date.now()}.xlsx`
        const buffer = await book.xlsx.writeBuffer()
        const url = await storageClient.addFile({
          filename,
          data: Buffer.from(buffer)
